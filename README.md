...
| 报告 | 结论摘要 |
|---|---|
| [`eval/RESULTS.md`](eval/RESULTS.md) | **扩语料到 900 片段（切分第一次真正运行），重测延迟发现瓶颈换人了**：BM25 增长 **40.5 倍**（超线性，根因是每次查询都把全部文档重新分词），而**余弦计算只占本地开销的 0.1%**。整个项目的性能叙事一直是「内存全量算余弦会慢」——**实测错了对象**。优化顺序因此完全反转，pgvector 从第一位掉到最后一位。 |
