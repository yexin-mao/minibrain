...
| 报告 | 结论摘要 |
|---|---|
| [`eval/RESULTS.md`](eval/RESULTS.md) | **pgvector + HNSW：把近似检索的价格量到了小数点后三位**。`ef=10` 一致率 87.9%、Recall 0.789→0.737，换 2.6 倍速度；`ef=20` 起召回零损失且快 2.9 倍。**但第一版测出来所有配置一模一样——两组对照都走了全表扫描**，因为优化器估算 HNSW 贵 40 倍（cost 2614 vs 65），实测反而快 3.2 倍。生产代码因此必须显式 `SET enable_seqscan = off`，否则建了索引也用不上**而且不报错**。 |
